...
class TTP_Image_Assy:
    ...
    # fixed: https://github.com/TTPlanetPig/Comfyui_TTP_Toolset/issues/31#issuecomment-3177610866
    def blend_tiles(self, tile1, tile2, overlap_size, direction, padding):
        ...
        # 修正：计算偏移量时确保裁剪尺寸一致（核心修复点）
        offset = (overlap_size - blend_size) // 2
        # 强制调整blend_size，避免因整数除法导致的尺寸偏差
        blend_size = overlap_size - 2 * offset  # 确保overlap_size = 2*offset + blend_size
        ...
        if direction == 'horizontal':
            # 计算裁剪区域（确保宽度一致）
            crop1_left = tile1.width - overlap_size + offset
            crop1_right = crop1_left + blend_size  # 强制宽度=blend_size
            crop_tile1 = tile1.crop((crop1_left, 0, crop1_right, tile1.height))

            crop2_left = offset
            crop2_right = crop2_left + blend_size  # 强制宽度=blend_size
            crop_tile2 = tile2.crop((crop2_left, 0, crop2_right, tile2.height))

            # 再次校验尺寸（可选，用于调试）
            if crop_tile1.size != crop_tile2.size:
                raise ValueError(f"水平裁剪尺寸不匹配: {crop_tile1.size} vs {crop_tile2.size}")

            blended = Image.composite(crop_tile1, crop_tile2, mask)
            result = Image.new("RGB", (tile1.width + tile2.width - overlap_size, tile1.height))
            result.paste(tile1.crop((0, 0, crop1_left, tile1.height)), (0, 0))
            result.paste(blended, (crop1_left, 0))
            result.paste(tile2.crop((crop2_right, 0, tile2.width, tile2.height)), (crop1_right, 0))
        else:
            # 垂直方向同理，强制高度一致
            crop1_top = tile1.height - overlap_size + offset
            crop1_bottom = crop1_top + blend_size  # 强制高度=blend_size
            crop_tile1 = tile1.crop((0, crop1_top, tile1.width, crop1_bottom))

            crop2_top = offset
            crop2_bottom = crop2_top + blend_size  # 强制高度=blend_size
            crop_tile2 = tile2.crop((0, crop2_top, tile2.width, crop2_bottom))

            # 再次校验尺寸（可选，用于调试）
            if crop_tile1.size != crop_tile2.size:
                raise ValueError(f"垂直裁剪尺寸不匹配: {crop_tile1.size} vs {crop_tile2.size}")

            blended = Image.composite(crop_tile1, crop_tile2, mask)
            result = Image.new("RGB", (tile1.width, tile1.height + tile2.height - overlap_size))
            result.paste(tile1.crop((0, 0, tile1.width, crop1_top)), (0, 0))
            result.paste(blended, (0, crop1_top))
            result.paste(tile2.crop((0, crop2_bottom, tile2.width, tile2.height)), (0, crop1_bottom))
        return result
